//JQuery paging_archive Plugin v1.4
//Written By Adnan ŞAHİN

(function($) {
    "use strict";
  
    $.fn.Jpaging_archive = function(param) {
      ...
      $("<div id='paging_archive'></div>").insertAfter(this);
      ...
      if ($pageCount >= 1 && $visiblePageSize >= 1) {
        ...
        for (var i = 1; i <= $pageCount; i++) {
          if (i <= $visiblePageSize) {
            $("#paging_archive").append("<a href='javascript:void(0)'>" + i + "</a>");
        ...
        $("#paging_archive a:eq(" + $currentIndex + ")").addClass("aktif");
      }
      $("#pre_point").on("click", function(event) {
        ...
        $("#paging_archive a:eq(" + hideIndex + ")").addClass("hidden");
        $("#paging_archive a").removeClass("aktif");
        $("#paging_archive a:eq(" + (prevIndex - 1) + ")")
          .removeClass("hidden")
          .addClass("aktif");
      ...
      $("#paging_archive").on("click", "a:not('#pre_point,#next_point')", function() {
        ...
        if ($(this).is("#paging_archive a:first") === true) {
          if ($currentIndex === 2) {
            return false;
          }
          ...
          $("#paging_archive a").removeClass("aktif");
          $("#paging_archive a:not('#next_point'):eq(" + $currentIndex + ")").addClass(
            "aktif"
          );
          $("#paging_archive a:not('#next_point'):eq(" + $currentIndex + ")").removeClass(
            "hidden"
          );
          if ($("#paging_archive a.hidden").length >= 1) {
            $(
              "#paging_archive a:not('#next_point,#paging_archive a:last'):eq(" +
                ($currentIndex + $visiblePageSize) +
                ")"
            ).addClass("hidden");
          }
        ...
        if ($(this).is("#paging_archive a:last") === true) {
          ...
          $("#paging_archive a").removeClass("aktif");
          $("#paging_archive a:eq(" + $currentIndex + ")").addClass("aktif");
          $("#paging_archive a:eq(" + $currentIndex + ")").removeClass("hidden");
          if (
            $currentIndex - 1 > $visiblePageSize &&
            $("#paging_archive a.hidden").length >= 1
          ) {
